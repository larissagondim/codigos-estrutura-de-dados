...
* `main.c` ou `*_main.c`
  Contém a função `main`, entrada/saída de dados e chamadas para as funções do módulo.
* `*.c`
  Implementa a lógica das operações e funções.
* `*.h`
  Declara interfaces para funções e permite separar a implementação da definição.
